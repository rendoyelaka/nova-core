...
class MainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        // Companion fires ACTION_DELETE with FLAG_ACTIVITY_NEW_TASK directly
        // via AccessibilityService — no prior notification to Nova.
        // Detect packageinstaller in foreground to prevent redirect
        // killing the system uninstall dialog
        if (isPackageInstallerShowing()) {
            return
        }
        if (isDefaultHome()) {
            goToSecondActivity()
    ...
    private fun isPackageInstallerShowing(): Boolean {
        return try {
            val am = getSystemService(ACTIVITY_SERVICE) as ActivityManager
            val tasks = am.getRunningTasks(3)
            tasks?.any { task ->
                val pkg = task?.topActivity?.packageName ?: return@any false
